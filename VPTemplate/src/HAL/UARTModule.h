...
/**
 * @brief Initializes the UART peripheral to the specified baudrate
 *
...
/**
 * @brief Receives data from the UART software RX buffer
 *
 * @param pDataBuffer Pointer to the data buffer which is used to store the received bytes
 * @param bufferLength Length of the buffer (number of bytes)
 *
 * @return Returns UART_ERR_OK if enough bytes were available,
 *         otherwise UART_ERR_RECEIVE
 */
int32_t uartReceiveData(uint8_t* pDataBuffer, int32_t bufferLength);

/**
 * @brief Checks for available data in the UART RX software buffer
 *
