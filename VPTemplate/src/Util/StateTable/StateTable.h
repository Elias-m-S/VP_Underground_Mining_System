...
#define STATETBL_ERR_EVENT_PENDING          -4      //!< New event sent but queue is full
...
/* Small event queue to avoid losing events within one cycle */
#define STT_EVENT_QUEUE_SIZE                8U
...
typedef struct _StateTable
{
    ...
    /* Compatibility field (was single pending event) */
    int32_t pendingEvent;                   //!< kept for compatibility, not used as single-slot anymore

    /* Event queue (ring buffer) */
    int32_t eventQueue[STT_EVENT_QUEUE_SIZE];
    uint8_t eventHead;
    uint8_t eventTail;
    uint8_t eventCount;
